...
def get_all_devices(
    ...
    limit: Optional[int] = Query(
        default=None, ge=1, le=1000, description="Maximum devices to return"
    ),
    offset: int = Query(
        default=0, ge=0, description="Number of devices to skip for pagination"
    ),
    include_archived: bool = Query(
        default=False, alias="includeArchived", description="Include archived devices"
    ),
):
    """
    ...
    try:
        ...
        if _cached is not None:
            add_pagination_headers(
                response, request, total=_cached["total"], limit=limit, offset=offset
            )
            return _cached
        ...
        for row in rows:
            ...
            device_platform = (
                system_os_name or platform or infer_platform(os_name or os)
            )
            ...
            if inventory_data_raw:
                try:
                    ...
                    if isinstance(raw_inventory, dict):
                        ...
                        normalized = {
                            "deviceName": raw_inventory.get("device_name")
                            or raw_inventory.get("deviceName"),
                            "assetTag": raw_inventory.get("asset_tag")
                            or raw_inventory.get("assetTag"),
                            "serialNumber": raw_inventory.get("serial_number")
                            or raw_inventory.get("serialNumber"),
                        ...
                        if summary:
                            ...
                            device_display_name = summary.get(
                                "deviceName", device_display_name
                            )
                except Exception as inventory_error:
                    logger.warning(
                        f"Failed to parse inventory data for {serial}: {inventory_error}"
                    )
            ...
            if network_data_raw:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to parse network data for {serial}: {network_error}"
                    )
            else:
            ...
            logger.info(
                f"[HOSTNAME_TEST_v2] Device {serial}: hostname_var={hostname}, network_summary_exists={bool(network_summary)}"
            )
        ...
        if devices:
            first_dev = devices[0]
            logger.info(
                f"[FINAL_RESPONSE_CHECK] First device: serial={first_dev.get('serialNumber')}, hostname={first_dev.get('hostname')}, has_modules_network={'network' in first_dev.get('modules', {})}"
            )
        ...
        add_pagination_headers(
            response, request, total=payload["total"], limit=limit, offset=offset
        )
...
def get_device_by_serial(serial_number: str):
    ...
    **Response includes:**
    ...
    try:
        ...
        if system_row:
            system_data = (
                json.loads(system_row[0])
                if isinstance(system_row[0], str)
                else system_row[0]
            )
        ...
        for table in module_tables:
            try:
                ...
                if table == "installs":
                    ...
                    cursor.execute(
                        f"SELECT data - 'runLog' FROM {table} WHERE device_id = %s",
                        (serial_num,),
                    )
                else:
                    cursor.execute(
                        f"SELECT data FROM {table} WHERE device_id = %s", (serial_num,)
                    )

                module_row = cursor.fetchone()
                if module_row:
                    module_data = (
                        json.loads(module_row[0])
        ...
        if modules.get("inventory"):
            ...
            if isinstance(inv, dict):
                display_name = (
                    inv.get("deviceName") or inv.get("device_name") or display_name
                )
        if (
            not display_name or display_name == "Unknown" or display_name == serial_num
        ) and modules.get("network"):
            net = modules["network"]
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve device: {str(e)}"
        )


@router.get(
    "/device/{serial_number}/installs/log",
    dependencies=[Depends(verify_authentication)],
    tags=["devices"],
)
def get_device_installs_log(serial_number: str):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve installs log: {str(e)}"
        )


@router.get(
    "/device/{serial_number}/logs/{tool}",
    dependencies=[Depends(verify_authentication)],
    tags=["devices"],
)
def get_device_log_root(serial_number: str, tool: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve log root: {str(e)}"
        )


@router.get(
    "/device/{serial_number}/events",
    dependencies=[Depends(verify_authentication)],
    tags=["devices"],
)
...
@router.get(
    "/device/{serial_number}/info",
    dependencies=[Depends(verify_authentication)],
    tags=["devices"],
)
def get_device_info_fast(serial_number: str):
    """
    ...
    try:
        ...
        logger.info(
            f"Fast info fetch for {serial_number}: {len(json.dumps(response))} bytes"
        )
        return response
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve device info: {str(e)}"
        )


@router.get(
    "/device/{serial_number}/modules/{module_name}",
    dependencies=[Depends(verify_authentication)],
    tags=["devices"],
)
def get_device_module(serial_number: str, module_name: str):
    """
    ...
    try:
        ...
        if module_name not in valid_modules:
            raise HTTPException(
                status_code=400, detail=f"Invalid module: {module_name}"
            )
        ...
        # Get module data
        cursor.execute(
            f"SELECT data FROM {module_name} WHERE device_id = %s", (serial_num,)
        )
        module_row = cursor.fetchone()
        ...
        if not module_row:
            return {"success": True, "module": module_name, "data": None}
        ...
        if (
            module_name == "system"
            and isinstance(module_data, list)
        ...
        response = {"success": True, "module": module_name, "data": module_data}

        logger.info(
            f"Module fetch {module_name} for {serial_number}: {len(json.dumps(response))} bytes"
        )
        return response
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve module: {str(e)}"
        )


@router.get(
    "/device/{serial_number}/applications/usage/history",
    dependencies=[Depends(verify_authentication)],
    tags=["devices"],
)
def get_device_usage_history(
    serial_number: str,
    days: int = Query(
        default=90, ge=1, le=548, description="Number of days to look back"
    ),
    app_name: Optional[str] = Query(
        default=None, alias="appName", description="Filter by application name"
    ),
):
    """
