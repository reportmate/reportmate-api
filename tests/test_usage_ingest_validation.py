...
These lock in the sanitization contract for usage rows: non-numeric and
non-finite durations are rejected, negative durations are clamped to zero
while the rest of the row is kept, malformed or implausible dates are
...
def test_valid_entry_passes_through():
    assert _usage_entry_numbers(_entry()) == (2, 3600.0, 1800.0, 1200.0, False)


def test_missing_optional_fields_default_to_zero():
    entry = {"date": "2026-08-03", "appName": "Safari"}
    assert _usage_entry_numbers(entry) == (0, 0.0, 0.0, 0.0, False)
...
def test_negative_duration_clamped_not_rejected():
    # macOS stamps -1 as an unknown-duration sentinel on sessions a watcher
    # restart interrupted, so a negative arrives alongside correct foreground
    # and active figures on ordinary healthy Macs. Dropping the row would
    # discard those; the clamp keeps them and flags the row instead.
    assert _usage_entry_numbers(_entry(totalSeconds=-1)) == (
        2,
        0.0,
        1800.0,
        1200.0,
    ...
    assert _usage_entry_numbers(_entry(foregroundSeconds=-1)) == (
        2,
        3600.0,
        1800.0,
        0.0,
    ...
    assert _usage_entry_numbers(_entry(launches=-3)) == (
        0,
        3600.0,
        1800.0,
        1200.0,
...
def test_clamp_flag_only_set_when_a_value_was_negative():
    assert _usage_entry_numbers(_entry())[4] is False
    assert _usage_entry_numbers(_entry(totalSeconds=0))[4] is False
...
def test_oversized_values_are_not_rejected_here():
    ...
    assert numbers == (2, 200000.0, 90000.0, 1200.0, False)
